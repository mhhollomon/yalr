...
namespace yalr {
    ...
    std::pair<bool, SymbolTable::symbol> 
    SymbolTable::insert(const ast::terminal& t) {

        auto iter = by_name.find(t.name);

        if (iter != by_name.end()) {
            return std::make_pair(false, iter->second);
        } 

        const auto new_ptr = std::make_shared<const symbol_imp>(++id_gen, t);
        auto new_sym = symbol{new_ptr};
        table.push_back(new_ptr);

        by_name.emplace(t.name, new_sym);
        by_id.emplace(id_gen, new_sym);
